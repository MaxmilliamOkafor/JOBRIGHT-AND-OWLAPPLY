...
(function () {
  ...
  function getLearnedAnswer(label, el) {
    ...
    // 2. Safe word-overlap match (prevents cross-field contamination)
  ...
  function guessValue(label, p) {
    ...
    if (/\bprefix\b|name.?prefix|salutation|honorific/.test(l)) return p.prefix || '';
    if (/\bsuffix\b|name.?suffix/.test(l)) return p.suffix || '';
    if (/number.*years|years.*number|how.*many.*years/.test(l)) return DEFAULTS.years;
    if (/proficiency|skill.?level|competency/.test(l)) return 'Advanced';
    ...
    if (/social.?security|ssn|national.?id|tax.?id/.test(l)) return '';
    return '';
  }
  ...
  function nativeSet(el, val) {
    ...
    // KeyboardEvent dispatch for masked inputs (phone formatters)
  ...
  function getLabel(el) {
    ...
    // data-automation-id / data-testid / data-qa
    ...
    // Parent text node (inline labels)
  ...
  async function fallbackFill() {
    ...
    for (const sel of selects) {
      ...
      const opts = $$('option', sel).filter(o => o.value && o.index > 0);
      // Exact text match → contains match → value attr match → word match
      ...
      if (!opt) {
        ...
        if (words.length) opt = opts.find(o => words.some(w => o.text.trim().toLowerCase().includes(w)));
      }
...
.ua-scrape-btn:disabled{background:#e5e7eb;color:#9ca3af;cursor:default}
  ...
  function bindDrawer() {
    ...
    const profFields = [
      ...
      { k: 'country', l: 'Country' }, { k: 'address', l: 'Address' }, { k: 'address_line_2', l: 'Address Line 2 (Apt/Suite)' },
      ...
      { k: 'school', l: 'School/University' }, { k: 'degree', l: 'Degree' }, { k: 'major', l: 'Major' }, { k: 'gpa', l: 'GPA' },
      ...
      { k: 'notice_period', l: 'Notice Period' }, { k: 'visa_status', l: 'Visa/Work Status' }, { k: 'skills', l: 'Skills' },
      { k: 'cover_letter', l: 'Cover Letter' }, { k: 'summary', l: 'Professional Summary' },
    ];
